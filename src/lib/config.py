...
Classes:
SpokeConfig - extends ConfigParser with a save method.

Functions:
setup - reads a configuration file and returns a SpokeConfig object
...
def setup(config_file=None, mco=False):
    """Instantiate (once only) and return SpokeConfig object."""
    global gConfig
...
# It may break plenty of other things, who knows! -MMB
    if gConfig is not None and mco == False:
        ...
        gConfig = SpokeConfig(config_file)
    return gConfig

class SpokeConfig(ConfigParser.ConfigParser):
